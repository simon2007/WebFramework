...
public class ImageUtil {
	...
	public static byte[] reSize(String orgFile, int height, int width) {
		File in = new File(orgFile);      //原图片
        ScaleParameter scaleParam = new ScaleParameter(width, height);  //将图像缩略到1024x1024以内，不足1024x1024则不做任何处理
        FileInputStream inStream = null;
...
//            outStream = new FileOutputStream(out);
	...
	public static byte[] reSize(byte[] orgBuff, int height, int width) {
        ...
        WriteRender wr = null;
        try {
            inStream = new ByteArrayInputStream(orgBuff);
            outStream = new ByteArrayOutputStream();
//            outStream = new FileOutputStream(out);
            ImageRender rr = new ReadRender(inStream);
            ImageRender sr = new ScaleRender(rr, scaleParam);
    ...
    public static byte[] transFormat(File orgFile,ImageFormat imageFormat ) {
        FileInputStream inStream = null;
        ByteArrayOutputStream outStream = null;
        WriteRender wr = null;
        try {
            inStream = new FileInputStream(orgFile);
            outStream = new ByteArrayOutputStream();
            ImageRender rr = new ReadRender(inStream);
            wr = new WriteRender(rr, outStream, imageFormat);
            wr.render();                            //触发图像处理
